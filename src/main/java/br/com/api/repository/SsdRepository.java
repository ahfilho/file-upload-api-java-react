...
//    @Query("SELECT x FROM Ssd x JOIN x.category c JOIN x.ImgSsd i")
//    public List<Ssd> teste();


//    @Query("SELECT x FROM Ssd x INNER JOIN Category c ON x.id = c.id INNER JOIN ImgSsd i ON x.id = i.id")
//    public List<Ssd> a();

//    @Query(value = "SELECT s FROM Ssd inner join category, inner join ImgSsd", nativeQuery = true)
